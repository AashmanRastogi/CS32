...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    vector<Store> auxStores(stores.begin(), stores.end());
      ...
      //        to point to the corresponding Store in auxStores. DONE
    ...
    for (int i = 0; i < auxStores.size(); i++)
    {
      ...
      //        with the comp parameter as the ordering relationship. DONE
    sort(v.begin(),v.end(),compareStorePtr);
      // TODO:  Using the now-sorted vector of pointers, replace each Store
      //        in stores with the Stores from auxStores in the correct order. DONE
    for (int i = 0; i < auxStores.size(); i++)
      {
        stores[i] = *v[i];
      }
